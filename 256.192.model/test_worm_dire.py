...
from networks import network_worm_direction
...
def main(args):
    ...
    # model = network_worm_direction.__dict__[cfg.model](cfg.output_shape, cfg.num_class, cfg.inter, pretrained = True)
    ...
    if True:
        with torch.no_grad():
            ...
            for i, (inputs, meta) in tqdm(enumerate(test_loader)):
                ...
                intersection_areas_numpy2 = 1.0 * (intersection_areas_numpy2 > 0.5)

                # end_points_areas_numpy = intersection_areas[0,3,:,:].cpu().detach().numpy() # endpoints
                # end_points_areas_numpy = 1.0 * (end_points_areas_numpy > 0.5) # endpoints

                
                # touching_areas_numpy = intersection_areas[0,2,:,:].cpu().detach().numpy() # touching
                # touching_areas_numpy = ((1.0 * (touching_areas_numpy > 0.2) - cv2.dilate(intersection_areas_numpy2, np.ones((5,5)))) > 0 ) * 1.0

                direcional_pred_numpy = np.zeros(direcional_pred_numpy.shape)
                direcional_pred_numpy = np.zeros((1, 6, 520, 696))
                img_name = meta['bi'][0]
                ...
                for each_dir in range(6):
                    ...
                    current_dir_image = np.pad(current_dir_image, ((4,4), (12,12)), 'constant')
                    ####################################

                    # current_dir_image = directional_pred[0,each_dir,:,:].cpu().detach().numpy()
                    current_dir_image = (current_dir_image>0.1) * 1.0


                    current_dir_image = current_dir_image * hard_mask
                    # current_dir_image = current_dir_image * (1-end_points_areas_numpy) * (1-touching_areas_numpy) 
                ...
                #     dirrectional_mask_one = (dirrectional_mask_one>0.1) * 1.0
                #     combined_show = draw_mask(image_show, dirrectional_mask_one)
                #     combined_show_touching = draw_mask(image_show, touching_areas_numpy)
                #     # cv2.imshow('dirrectional_mask pred',combined_show)
                #     # cv2.imshow('combined_show_touching',combined_show_touching)
                #     # cv2.waitKey(0)
                # mask ######################################################################3:
                ...
                # combined_show_end_points_areas_numpy = draw_mask(image_show, end_points_areas_numpy)
                ...
                # cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) +"combined_show_end_points_areas_numpy.png" ), combined_show_end_points_areas_numpy*255.)
                ...
                segments = (hard_mask - intersection_areas_numpy2 - mask_pred_numpy_2 ) > 0 
