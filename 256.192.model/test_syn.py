...
def main(args):
    ...
    # demo_folder = "./demo_folder/microtubule/"
    # result_path = "./demo_folder/results/"
    # os.makedirs(result_path, exist_ok=True)

    # result_file = os.path.join(result_path, 'result_5.json')
    # result_file = os.path.join(result_path, 'microtubule.json')
        ...
        with torch.no_grad():
            ...
            for i, (inputs, meta) in tqdm(enumerate(test_loader)):
                ...
                def mask_from_seeds_v3(embedding, island, mask, segments, ds, similarity_thres=1.0):
                    ...
                    seperating_island = np.zeros(segments.shape)
                    seperating_island_id = 1
                    ...
                    for eachisland in np.unique(island):
                        ...
                        check_map = deepcopy(current_island)

                        # get connection areas 
                        # 1. dilate junction
                        connection_areas = (cv2.dilate(current_island.astype('uint8'), np.ones((15,15))) * mask - cv2.dilate(current_island.astype('uint8'), np.ones((3,3))) > 0 ) * 1.0
                        # 2. find pixels overlap with segments
                        connection_areas_label = (connection_areas>0) * segments

                        canvas_for_seperating_island = deepcopy(connection_areas_label)
                        ...
                        while True:
                            ...
                            dilated = im_dilation(canvas_for_seperating_island, mor_square(5)) 

                            front_r, front_c = np.nonzero((check_map + connection_areas) * (canvas_for_seperating_island != dilated))
                            # similarity = [np.sqrt(embedding_copy[r, c] - mean_vals[dilated[r, c]])
                            #             for r, c in zip(front_r, front_c)]
                            similarity = [np.abs(embedding_copy[r, c] - mean_vals[dilated[r, c]])
                                        for r, c in zip(front_r, front_c)]

                            # bg = seeds[front_r, front_c] == 0
                            # add_ind = np.logical_and([s > similarity_thres for s in similarity], bg)
                            add_ind = np.array([s < 1.0 for s in similarity])

                            if np.all(add_ind == False) or np.sum(check_map) == 0:
                                break

                            canvas_for_seperating_island[front_r[add_ind], front_c[add_ind]] = dilated[front_r[add_ind], front_c[add_ind]]
                            check_map[front_r[add_ind], front_c[add_ind]] = 0
                        ...
                        for connector_id, to_be_connected in enumerate(col_ind):
                            ...
                            island_maping[segments_a_label].append(seperating_island_id)
                            island_maping[segments_b_label].append(seperating_island_id)
                            ds.add(segments_a_label, segments_b_label)

                            seperating_island[canvas_for_seperating_island==connect_label_a] = seperating_island_id
                            seperating_island[canvas_for_seperating_island==segments_b_label] = seperating_island_id
                    ...
                    return island_maping, ds, seperating_island
                ...
                under contstruction: 
                # build connector relations with island
                island_labels = label(intersection_areas_numpy2)
                island_to_connectors = {}
                for _, unique_island_label in enumerate(np.unique(island_labels)):
                    if unique_island_label == 0:
                        continue
                    current_island = (island_labels == unique_island_label) * 1.0
                    dilated_island = cv2.dilate(current_island, np.ones((5,5)))
    ...
    ###################################################################
                ...
                island_mapping, grouped_ds, seperating_island_map = mask_from_seeds_v3(tags_map_final, islands_labels, mask_pred_numpy, segments_labels, ds)
                ...
                for old_id in old_to_new_mapping.keys():
                    ...
                    result_map_dilated[np.isin(seperating_island_map, island_groups)] = new_id

                    temp = instance_map_recorder[new_id - 1]
                    # temp[np.isin(islands_labels, island_groups)] = 255.
                    temp[np.isin(seperating_island_map, island_groups)] = 255.
