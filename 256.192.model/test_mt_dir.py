...
def main(args):
    ...
    demo_folder = os.path.join(result_folder, "intermediate_results")
    segments_map_folder = os.path.join(result_folder, "segments_map")
    segments_result_map_di =  os.path.join(result_folder,"segments_result_map_di")
    segments_result_map =  os.path.join(result_folder,"segments_result_map")
    result_path =  os.path.join(result_folder,"results")

    os.makedirs(result_path, exist_ok=True)
    os.makedirs(segments_map_folder, exist_ok=True)
    os.makedirs(segments_result_map_di, exist_ok=True)
    os.makedirs(segments_result_map, exist_ok=True)
    ...
    demo_folder_connection_areas = os.path.join(result_folder, "synthetic_connection_areas")
    ...
    if True:
        with torch.no_grad():
            mt_files = os.listdir( '/home/yliu/work/data/microtubule_data/binary/')
            mt_files = os.listdir( '/home/yliu/work/data/zder_s_cropped_1024/')
            ...
            for i, name in tqdm(enumerate(mt_files)):
                ...
                # img_path = os.path.join('/home/yliu/work/data/microtubule_data/binary/',mt_files[i])

                img_path = os.path.join('/home/yliu/work/data/zder_s_cropped_1024/',mt_files[i])

                oirien_folder = "/home/yliu/work/colab/work/fiberPJ/pytorch-cpn/256.192.model/six_output/mts_dspredict"
                oirien_folder = "/home/yliu/work/colab/work/fiberPJ/pytorch-cpn/256.192.model/six_output/mt_12_images"
                # oirien_folder = "/home/yliu/work/colab/work/fiberPJ/pytorch-cpn/256.192.model/six_output/pruberson_images_512/"
                oirien_folder = "/home/yliu/work/colab/work/fiberPJ/pytorch-cpn/256.192.model/six_output/pruberson_images_1024/"
                
                image = imageio.imread(img_path)


                img_name = mt_files[i].split('.')[0]

                fpath_merge = img_name + '_branches_merge' + ".png"
                image = imageio.imread(os.path.join(oirien_folder, fpath_merge))
                ...
                direcional_pred_numpy = np.zeros(directional_pred.shape)
                reconstruction_numpy_fordebug = np.zeros(hard_mask.shape)
                ...
                for each_dir in range(6):
                    ...
                    current_dir_image = cv2.dilate((current_dir_image > 0 )*1.0, np.ones((5,5))) 
                    ...
                    # direcional_pred_numpy[0, each_dir, :,:] = current_dir * cv2.erode((1- (no_cross_area > 0) * 1.0), np.ones((3,3)))

                    no_cross_area += (current_dir> 0) * 1.0
                    # import pdb; pdb.set_trace()
                    # dirrectional_mask_one = current_dir
                    ...
                    # combined_show_inter =  draw_mask(current_dir_image, intersection_areas_numpy2_dir)
                    # combined_show_dirrectional_mask_one = draw_mask(image_show, current_dir * 255.)
                    # reconstruction_numpy_fordebug += (direcional_pred_numpy[0, each_dir, :,:] > 0) * 1.0
                    # combined_show_reconstruction_numpy_fordebug = visualize_label_map( image, reconstruction_numpy_fordebug)
                    ...
                    # cv2.imshow('combined_show_reconstruction_numpy_fordebug',combined_show_reconstruction_numpy_fordebug)
                    # cv2.waitKey(0)

                    # current_dir = (current_dir[:,:,0] > 0 )*1.0
                    direcional_pred_numpy[0, each_dir, :,:] = current_dir 
                import copy
                direcional_pred_numpy_for_indexing =  copy.deepcopy(direcional_pred_numpy)
                for each_dir in range(6):

                    direcional_pred_numpy[0, each_dir, :,:] = direcional_pred_numpy[0, each_dir, :,:] * (1 - cv2.dilate((no_cross_area > 1).astype(np.uint8), np.ones((3,3))) ) 
                    # reconstruction_numpy_fordebug =  (direcional_pred_numpy[0, each_dir, :,:])
                    # combined_show_reconstruction_numpy_fordebug = visualize_label_map( image, reconstruction_numpy_fordebug)
                    # cv2.imshow('combined_show_reconstruction_numpy_fordebug_v2',combined_show_reconstruction_numpy_fordebug)
                    # cv2.waitKey(0)
                ...
                def connect_with_dir(directional_map, segments, connectors, island, mask_pred, ds):
                    thread_mapping = defaultdict(list)
                    used_connectors = []
                    for ddi in range(6):
                        ...
                        # label_directional_mask_one = im_dilation(label_directional_mask_one, np.ones((3,3)))
                        ...
                        for each_sample in np.unique(label_directional_mask_one):
                            ...
                            thread_mapping[base].append((ddi, each_sample))
                    ...
                    return ds, used_connectors, connectors, thread_mapping
    ...
    ###################################################################
                ...
                islands_labels_dilate = (im_dilation(islands_labels, mor_square(7)) * hard_mask  ).astype('int')
                ...
                cv2.imwrite(os.path.join(demo_folder,str(i) +"_1_0_before_connectors.png" ), visualize_imgconnector_labels)
                # cv2.imshow('t',visualize_img )
                # cv2.imshow('visualize_imgconnector_labels',visualize_imgconnector_labels )
                # cv2.waitKey(0)

                grouped_ds, used_connectors, connector_labels_updated, thread_mapping = connect_with_dir(direcional_pred_numpy,segments_labels,connector_labels, islands_labels_dilate, mask_pred_numpy, ds)
                
                grouped_ds_with_dir = deepcopy(grouped_ds)
                ...
                cv2.imwrite(os.path.join(demo_folder,str(i) +"_1_1after_connectors.png" ), visualize_connector_labels_updated)
                # cv2.imshow('t',visualize_img )
                # cv2.imshow('visualize_connector_labels_updated',visualize_connector_labels_updated )
                # cv2.waitKey(0).

                sim_thre = 0.0001
                island_mapping, grouped_ds =  mask_from_seeds_v4(tags_map_final, islands_labels_dilate, mask_pred_numpy, segments_labels, grouped_ds, connector_labels_updated, sim_thre)
                ...
                grouped_ds_with_dir_groups = grouped_ds_with_dir.group
                ...
                for key, group_set in grouped_ds_with_dir_groups.items():
                ...
                cv2.imwrite(os.path.join(segments_result_map_di,mt_files[i][:-4] + "_labeled_segments_with_direction.png"),result_map_segments_for_save)
                ...
                for key, group_set in groups.items():
                    ...
                    canvas = np.zeros(segments.shape)
                    canvas[np.isin(segments_labels, group_as_array)] = 255.
                    canvas = im_dilation(canvas, mor_square(5)) * hard_mask  
                    instance_map_recorder.append(canvas)
                ...
                cv2.imwrite(os.path.join(segments_result_map,mt_files[i][:-4] + "_labeled_segments.png"),result_map_segments_for_save)

                # add thread
                for original_segment_id in thread_mapping.keys():
                    leader_id = grouped_ds.find(original_segment_id)

                    new_id = old_to_new_mapping[leader_id]
                    for each_tuple in thread_mapping[original_segment_id]:
                        page_id, thread_id = each_tuple[0], each_tuple[1]
                        result_map_dilated[direcional_pred_numpy_for_indexing[0][page_id] == thread_id] = new_id

                result_map_segments_for_save = visualize_label_map(np.tile(result_map_segments_canvas*255., (3,1,1)).transpose(1,2,0), result_map_dilated)
                
                cv2.imwrite(os.path.join(segments_result_map,mt_files[i][:-4] + "_labeled_segments_thread.png"),result_map_segments_for_save)                
