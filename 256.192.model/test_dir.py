...
from networks import network_direction
...
from utils.get_intersection_and_endpoint import get_skeleton_endpoint, get_skeleton_intersection_and_endpoint, get_skeleton_intersection
...
def main(args):
    ...
    model = network_direction.__dict__[cfg.model](cfg.output_shape, cfg.num_class, cfg.inter, pretrained = True)
    ...
    if True:
        with torch.no_grad():
            ...
            for i, (inputs, meta) in tqdm(enumerate(test_loader)):
                ...
                direcional_pred_numpy = directional_pred.cpu().detach().numpy()
                direcional_pred_numpy_predicted = (direcional_pred_numpy > 0.2) * 1.0

                direcional_pred_numpy = np.zeros(direcional_pred_numpy.shape)
                img_name = meta['img_path'][0]
                img_name= img_name.split('/')[-1].split('.')[0]

                
                # no_cross_area = np.zeros(input_image[:,:,0] .shape)
                for each_dir in range(6):
                    fpath_direction = img_name + '_branch_' + str(each_dir+1) + ".png"

                    current_dir_image = cv2.imread(os.path.join(oirien_folder, fpath_direction))

                    current_dir_image = cv2.dilate((current_dir_image > 0.5 )*1.0, np.ones((2,2))) 

                    from skimage.morphology import skeletonize
                    current_dir_image = current_dir_image[:,:,0]





                    current_dir_image = current_dir_image * hard_mask
                    current_dir_image = skeletonize(current_dir_image>0)
                    intersection_points = get_skeleton_intersection(current_dir_image, remove_closed_ones=False)
                    intersections_mask = np.zeros(current_dir_image.shape)
                    for p in intersection_points:
                        intersections_mask[p[1], p[0]] = 1

                    intersections_mask = cv2.dilate(intersections_mask, np.ones((5,5)))

                    current_dir_image = current_dir_image * (1 - intersections_mask)
                    # current_dir = current_dir_image[:,:,0]
                    current_dir = current_dir_image

                    current_dir = label(current_dir)
                    current_dir_thin_visual = label(current_dir)
                    current_dir = im_dilation(current_dir, np.ones((3,3)))

                    # direcional_pred_numpy[0, each_dir, :,:] = current_dir * cv2.dilate((1- (no_cross_area > 0) * 1.0), np.ones((3,3)))
                    direcional_pred_numpy[0, each_dir, :,:] = current_dir

                    # no_cross_area += (current_dir> 0) * 1.0
                    # combined_show_dirrectional_mask_one = visualize_label_map(image_show, no_cross_area)
                    combined_show_current_dir = visualize_label_map(np.tile(hard_mask*255., (3,1,1)).transpose(1,2,0), current_dir)

                    combined_show_current_dir_thin_visual = visualize_label_map(np.tile(hard_mask*255., (3,1,1)).transpose(1,2,0), current_dir_thin_visual)

                    cv2.imwrite(os.path.join(result_for_six_output_folder,str(i) + "_"+str(each_dir)+ "_" + "direcetional_prediction.png" ), cv2.dilate(current_dir_image, np.ones((3,3)))*255.)
                    cv2.imwrite(os.path.join(result_for_six_output_folder,str(i) + "_"+str(each_dir)+ "_" + "direcetional_prediction_no_dilate.png" ), cv2.dilate(current_dir_image, np.ones((1,1)))*255.)
                    cv2.imwrite(os.path.join(result_for_six_output_folder,str(i) + "_"+str(each_dir)+ "_" + "labeled_direcetional_prediction.png" ), combined_show_current_dir)
                    cv2.imwrite(os.path.join(result_for_six_output_folder,str(i) + "_"+str(each_dir)+ "_" + "combined_show_current_dir_thin_visual.png" ), combined_show_current_dir_thin_visual)

                    # cv2.imshow("t", combined_show_dirrectional_mask_one)
                    # cv2.imshow("tt", combined_show_current_dir)
                    # cv2.imshow("current_dir_image", current_dir_image * 255.)

                    
                    # cv2.waitKey(0)


                # read from orientation-aware


                # for ddi in range(6):
                #     print(ddi)
                #     dirrectional_mask_one = directional_pred[0,ddi,:,:].cpu().detach().numpy()
                #     dirrectional_mask_one = cv2.erode((dirrectional_mask_one>0.5).astype("uint8"), np.ones((2,2))) * 255.0 * hard_mask 
                #     dirrectional_mask_one_visual = dirrectional_mask_one
                #     if len(dirrectional_mask_one.shape) == 2:
                #             dirrectional_mask_one = np.tile(dirrectional_mask_one,(3,1,1)).transpose(1,2,0)
                #             dirrectional_mask_one = np.asarray(dirrectional_mask_one) 
                            

                #     dirrectional_mask_one = im_to_torch(dirrectional_mask_one)
                #     dirrectional_mask_one = color_normalize(dirrectional_mask_one, cfg.pixel_means)
                #     inputs_dirrectional_mask_one = dirrectional_mask_one.unsqueeze(0)

                #     input_var_dirrectional_mask_one = torch.autograd.Variable(inputs_dirrectional_mask_one.to(device))

                #     a,b,c, intersection_areas_dir  = model(input_var_dirrectional_mask_one)

                #     intersection_areas_numpy2_dir = intersection_areas_dir[0,1,:,:].cpu().detach().numpy() # isalnd
                #     intersection_areas_numpy2_dir = (intersection_areas_numpy2_dir > 0.9) *255.0

                #     combined_show_inter =  draw_mask(image_show, intersection_areas_numpy2_dir)
                #     combined_show_dirrectional_mask_one = draw_mask(image_show, dirrectional_mask_one_visual)
                #     # combined_show_dirrectional_mask_one = visualize_label_map(image_show,dirrectional_mask_one_visual)
                #     cv2.imshow('combined_show_mask_pred_numpy_2',combined_show_inter)
                #     cv2.imshow('combined_show_dirrectional_mask_one',combined_show_dirrectional_mask_one)
                #     cv2.imshow('dirrectional_mask_one_visual',dirrectional_mask_one_visual)
                ...
                intersection_areas_numpy2 = (intersection_areas_numpy2 > 0.7) *1.0
                # cv2.imshow('intersection_areas_numpy2',intersection_areas_numpy2*255.)
                # cv2.waitKey(0)
                ...
                # embedding map only show intersection area
                ...
                def connect_with_dir(directional_map, segments, connectors, island, mask_pred, ds):
                    used_connectors = []
                    for ddi in range(6):
                        label_directional_mask_one =  directional_map[0,ddi,:,:]

                        # directional_mask_one = directional_map[0,ddi,:,:] * mask_pred
                        # directional_mask_one = cv2.erode(directional_mask_one, np.ones((2,2)))
                        # # cv2.imshow('directional_mask_one',directional_mask_one * 255.)
                        # # from skimage.morphology import skeletonize
                        # # directional_mask_one = skeletonize(directional_mask_one)
                        # # cv2.imshow('skeletonize',directional_mask_one * 255.)
                        # directional_mask_one = remove_small_objects(directional_mask_one > 0, 10)
                        # label_directional_mask_one = label(directional_mask_one)
                        # visualize_img = visualize_label_map(np.tile(mask_pred_numpy*255., (3,1,1)).transpose(1,2,0), label_directional_mask_one)
                        # cv2.imshow('connecting_segments',visualize_img)
                        # cv2.waitKey(0)
                        for each_sample in np.unique(label_directional_mask_one):
                            if each_sample ==0 :
                                continue
                            overlap_pixels = segments[label_directional_mask_one == each_sample]

                            connectors_overlap_pixels = connectors[label_directional_mask_one == each_sample]
                            list_of_connector_id = np.unique(connectors_overlap_pixels)
                            list_of_connector_id = np.delete(list_of_connector_id, np.where(list_of_connector_id==0))   


                            # all_comb = np.meshgrid(list_of_connector_id,list_of_connector_id)                 
                            # import pdb; pdb.set_trace()

                            list_of_segments_id = np.unique(overlap_pixels)
                            list_of_segments_id = np.delete(list_of_segments_id, np.where(list_of_segments_id==0))
                            if len(list_of_segments_id) <= 1:
                                continue
                            base = list_of_segments_id[0]
                            for each_segment_id in list_of_segments_id:
                                
                                ds.add(base, each_segment_id)

                            record_matching_connectors = defaultdict(list)
                            for each_connector_id in list_of_connector_id:
                                island_id = island[connectors == each_connector_id]

                                if np.isin(0,island_id):
                                    island_id = np.delete(island_id,np.where(island_id==0))

                                if len(island_id) == 0:
                                    continue                                
                                island_id = np.bincount(island_id).argmax()

                                if len(record_matching_connectors[island_id])==1:
                                    popped_connector_id = record_matching_connectors[island_id].pop()
                                    
                                    connectors[connectors == popped_connector_id] = 0
                                    connectors[connectors == each_connector_id] = 0

                                    used_connectors.append(popped_connector_id)
                                    used_connectors.append(each_connector_id)
                                else:
                                    record_matching_connectors[island_id].append(each_connector_id)


                    return ds, used_connectors, connectors
                ...
                def mask_from_seeds_v3(embedding, island, mask, segments, ds, similarity_thres=1.0):
                    ...
                    for eachisland in np.unique(island):
                        ...
                        # 1. dilate junction
                        connection_areas = (cv2.dilate(current_island.astype('uint8'), np.ones((9,9))) * mask - cv2.dilate(current_island.astype('uint8'), np.ones((5,5))) > 0 ) * 1.0
                ...
                def mask_from_seeds_v4(embedding, island, mask, segments, ds, connector_labels, similarity_thres=1.0):

                    # connector_props = regionprops(connector_labels)
                    ...
                    for eachisland in np.unique(island):
                        ...
                        connection_areas_label = 1.0 * (connector_labels > 0 ) * (island == eachisland) * segments
                        connection_areas_label = connection_areas_label.astype(int)
                ...
                # filament with width 3
                # segments = (mask_pred_numpy - binary_dilation(intersection_areas_numpy2, structure=np.ones((3,3)))  - mask_pred_numpy_2 * binary_dilation(intersection_areas_numpy2, structure=np.ones((5,5)))) > 0 
                segments = remove_small_objects(segments, 16)
                segments_labels = label(segments, connectivity=1)
                ...
                connector_labels = label(visual_connector)
                visual_connector_labels = connector_labels




                visual_connector_labels = visualize_label_map(np.tile(mask_pred_numpy*255., (3,1,1)).transpose(1,2,0), visual_connector_labels)
                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) + "visual_connector_labels.png" ), visual_connector_labels)
                ...
                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) + "image_intersection_labels.png" ), visualize_image_intersection_labels)
                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) +"image_dilation.png" ), visual_image_intersection_labels_dilation)
                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) +"inte_sub.png" ), visual_intersection_labels_dilation_sub_visual)
                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) +"segments_labels.png" ), visual_segments_labels_visual)
                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) +"connection_areas_visual.png" ), visual_connection_areas_visual)
    ...
    ###################################################################
                ...
                seeds = (seeds * 500 + intersection_labels_dilation * connection_areas).astype(int)
                props = regionprops(seeds)
                ...
                islands = cv2.dilate(intersection_areas_numpy2, np.ones((3,3)))
                
                islands_labels = label(islands)
                islands_labels_dilate = (im_dilation(islands_labels, mor_square(5)) * hard_mask  ).astype('int')
                visualize_img = visualize_label_map(np.tile(mask_pred_numpy*255., (3,1,1)).transpose(1,2,0), islands_labels_dilate)
                visualize_imgconnector_labels = visualize_label_map(np.tile(mask_pred_numpy*255., (3,1,1)).transpose(1,2,0), connector_labels)

                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) +"_1_before_connectors.png" ), visualize_imgconnector_labels)
                # cv2.imshow('t',visualize_img )
                # cv2.imshow('visualize_imgconnector_labels',visualize_imgconnector_labels )
                # cv2.waitKey(0)

                ####################################################
                grouped_ds, used_connectors, connector_labels_updated = connect_with_dir(direcional_pred_numpy,segments_labels,connector_labels, islands_labels_dilate, mask_pred_numpy, ds)
                
                visualize_connector_labels_updated = visualize_label_map(np.tile(mask_pred_numpy*255., (3,1,1)).transpose(1,2,0), connector_labels_updated)
                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) +"_1_after_connectors.png" ), visualize_connector_labels_updated)


                # cv2.imshow('t',visualize_img )
                # cv2.imshow('visualize_connector_labels_updated',visualize_connector_labels_updated )
                # cv2.waitKey(0)
                sim_thresh = 1.0
                # sim_thresh = 0.0000001
                island_mapping, grouped_ds =  mask_from_seeds_v4(tags_map_final, islands_labels_dilate, mask_pred_numpy, segments_labels, grouped_ds, connector_labels_updated, sim_thresh)


                ##############################################################
                # island_mapping, grouped_ds = mask_from_seeds_v3(tags_map_final, islands_labels, mask_pred_numpy, segments_labels, ds,similarity_thres=1.0)
                ...
                result_map_dilated = im_dilation(result_map, mor_square(3)) * mask_pred_numpy    
                ...
                cv2.imwrite(os.path.join(final_result_folder,str(i) +"result_map_dilated.png" ), visualize_img)
                cv2.imwrite(os.path.join(intermeidate_output_folder,str(i) +"result_map.png" ), result_map_visualize_img)
                cv2.imwrite(os.path.join(final_result_folder,str(i) +"result_map.png" ), result_map_visualize_img)
