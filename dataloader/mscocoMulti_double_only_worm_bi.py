...
class MscocoMulti_double_only_worm(data.Dataset):
    def __init__(self, cfg, train=True):
        ...
        self.demo_folder = "/home/yliu/work/colab/work/fiberPJ/pytorch-cpn/256.192.model/demo_folder/training_data_worm"
    ...
    def __getitem__(self, index):
        ...
        # if True:
            ...
            intersection_areas_small = cv2.dilate((segmentation_mask_final > 1).astype("uint8"), np.ones((3,3))) * ((segmentation_mask_final>0) * 1.)
            ...
            if self.inter:
                ...
                if np.sum(segmentation_mask_final>1) != 0:
                    ...
                    segmentation_mask_final_inter_globs = ((((distance_values < 25) * (distance_values > 0) * 1.0 + 1.0 * (segmentation_mask_final>1)) * (segmentation_mask_final > 0) ) > 0)* 1.0 
            ...
            image_w1 = cv2.imread(w1_file_path, cv2.COLOR_BGR2GRAY)
            binary_image = cv2.imread(binary_path, cv2.COLOR_BGR2GRAY)
        ...
        # image = np.asarray(segmentation_mask_final)
